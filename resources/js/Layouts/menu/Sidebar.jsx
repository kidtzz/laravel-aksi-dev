...
export default function Sidebar() {
  ...
  // const [actived, setAtived] = useState(false); // hrus ada flag halaman 
  ...
  // useEffect(
  //   () => {
  //     let timer1 = setTimeout(() => setOpen(false), 50000 * 1000);
  //     return () => {
  //       clearTimeout(timer1);
  //     };
  //   }, []);
      ...
      {/* <aside id="layout-menu" className="layout-menu menu-vertical menu bg-menu-theme">
        ...
        <ul className="menu-inner py-1" >
          {menuItems.map((i) => {
            return (
              <>
                {Array.isArray(i.menu) ? i.menu.map((i2) => {
                  if (i2.menuHeader) {
                    return (
                      <>
                        <li li className="menu-header small text-uppercase" > <span className="menu-header-text">{i2.menuHeader}</span>
                        </li>
                      </>
                    )
                  } else if (i2.name) {
                    if (i2.submenu) {
                      return (
                        <>
                          <li className={`menu-item ${openClass} ${actived}`} onClick={handleOpen}>
                            <a href={i2.url} className="menu-link menu-toggle">
                              <i className={`menu-icon' ${i2.icon}`}></i>
                              <div data-i18n={i2.name}>{i2.name}</div>
                            </a>
                            {open ? (
                              <ul className="menu-sub">
                                <>
                                  {Array.isArray(i2.submenu) ? i2.submenu.map((i3, index) => {
                                    console.log("i3?", i3)
                                    return (
                                      <li className={`menu-item ${actived}`} key={index} onClick={handleActive}>
                                        <a href={i3.url} className="menu-link">
                                          <div data-i18n={i3.name}>{i3.name}</div>
                                        </a>
                                      </li>
                                    )
                                  }) : ""
                      ...
                      return (
                        <li className={`menu-item`} >
                          <a href={i2.url} className="menu-link">
                            <i className={`menu-icon' ${i2.icon}`}></i>
                            <div data-i18n={i2.name}>{i2.name}</div>
                          </a>
                        </li>
